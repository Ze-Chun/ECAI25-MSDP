...
class Executor:
    ...
    def get_map(self, caption: str, sm, box_num):
        for i in range(box_num):
            file_path = '/media/xxx/FGVP/fgvp-reclip/output/visual_ini/{}/circle{}.jpg'.format(caption, i)
            img = Image.open(file_path)
            cv2_img = cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR)
            cv2_img = cv2.resize(cv2_img, (384, 384))
            # print(sm)
            # print(sm.shape)
            chunks = sm.chunk(box_num, dim=2)
            for j, chunk in enumerate(chunks):
                if j != i:
                    continue
                similarity_map = self.get_similarity_map(chunk[:, 1:, :], cv2_img.shape[:2])
                # print(similarity_map.shape)
                for b in range(similarity_map.shape[0]):
                    for n in range(similarity_map.shape[-1]):
                        # vis = np.clip(similarity_map[b, :, :, n].cpu().numpy() * 255, 0, 255).astype('uint8')
                        ...
                        vis = cv2_img * 0.45 + vis * 0.55
                        vis = cv2.cvtColor(vis.astype('uint8'), cv2.COLOR_BGR2RGB)
                        print()
                        plt.title(caption)
                        plt.imsave('/media/xxx/FGVP/fgvp-reclip/output/visual_ini/{}/circle{}_similarity.jpg'.format(caption, i), vis)
                        # input()
        ...
        if self.cache_path is None or any([not os.path.exists(os.path.join(self.cache_path, model_name, image_name, method_name+".pt")) for model_name in self.model_names for method_name in self.box_representation_method.split(',')]):
            ...
            if "blur" in self.box_representation_method:
                for i in range(len(boxes)):
                    ...
                    # ######## #
                    if not os.path.exists("./image/{}".format(caption)):
                        os.makedirs("./image/{}".format(caption))
                    blurred.save('./image/{}/blur{}.jpg'.format(caption, i))
                    # ######## #
            ...
            if "shade" in self.box_representation_method:
                for i in range(len(boxes)):
                    ...
                    if args.clip_processing == 'padding_R':
                        shaded_image = add_padding(shaded_image)
            ...
            if "fine-grained" in self.box_representation_method:
                ...
                if 'none' in args.sam_prompt:
                    ...
                    if args.recompute_box:
                        ...
                        centers = torch.stack([boxes[:, 0::2].mean(1), boxes[:, 1::2].mean(1)], 1)
                clip_inputs = torch.cat([self.fgvp(vp, image, centers, boxes, masks, caption) for vp in args.visual_prompt])
    ...
    def __call__(self, caption: str, image: Image, boxes: List[Box], image_name: str = None, caption_bank=[]) -> torch.Tensor:
        ...
        for model, images_t, model_name in zip(self.models, images, self.model_names):
            ...
            logits_per_image, logits_per_text, image_features, text_features, sm_cs_per_text = self.call_model(model, images_t, text_tensor, image_features=image_features, text_features=text_features)
            all_logits_per_image.append(logits_per_image)
            all_logits_per_text.append(logits_per_text)

            # **************************** #
            # print('-')
            self.get_map(caption, logits_per_text, len(boxes))
            # **************************** #
        ...
        # **************************** #
        # self.get_map(caption, all_logits_per_text.sum(dim=0, keepdim=True), len(boxes))
        # **************************** #
        # 1 * n_prompt, n_boxes
...
class FLAVAExecutor(Executor):
    def __init__(self, args=None, flava_model: str = "/media/xxx/FGVP/fgvp-reclip/flava", device: str = "cpu", box_representation_method: str = "crop", method_aggregator: str = "max", enlarge_boxes: int = 0, expand_position_embedding: bool = False, square_size: bool = False, blur_std_dev: int = 100, cache_path: str = None, enable_lora: bool = False, lora_path: str = None) -> None:
        ...
        model = FlavaModel.from_pretrained(flava_model)
        # if enable_lora:
        #     config = LoraConfig(
        #         r=16,
        #         lora_alpha=16,
    ...
    def preprocess_text(self, text: str) -> torch.Tensor:
        inputs = self.tokenizer([t.lower() for t in text], return_tensors="pt", padding="max_length", max_length=77)
        return inputs
    ...
    def call_model(self, model: torch.nn.Module, images: torch.Tensor, text: Dict, image_features: torch.Tensor = None, text_features: torch.Tensor = None) -> torch.Tensor:
        ...
        if self.args.TD:
            ...
            num_text = text['input_ids'].shape[0]
            if "shade" in self.box_representation_method:
                redundant_features = model.encode_text(self.preprocess_text([" is in red color."]*num_text).to(self.device))
            elif self.args.text_prompt == 'a photo of <caption>':
                redundant_features = model.get_text_features(**self.preprocess_text(["a photo of "]*num_text).to(self.device))[:, 0]
            elif self.args.text_prompt == 'This is <caption>':
                redundant_features = model.encode_text(self.preprocess_text(["This is "]*num_text).to(self.device))
            elif self.args.text_prompt == 'None':
                redundant_features = model.encode_text(self.preprocess_text([""]*num_text).to(self.device))
            elif self.args.text_prompt == 'a photo of a <caption>':
                redundant_features = model.get_text_features(self.preprocess_text(["a photo of a"]*num_text).to(self.device))[:, 0]
    ...
    def __call__(self, caption: str, image: Image, boxes: List[Box], image_name: str = None, caption_bank=[]) -> torch.Tensor:
        if "shade" in self.box_representation_method:
            ...
            caption = f"This is {caption.lower()}"
                ...
                caption = f"{caption.lower()}"
        elif self.args.text_prompt == 'a photo of a <caption>':
                caption = f"a photo of a {caption.lower()}"
        else:
            raise NotImplementedError
...
class ClipExecutor(Executor):
    ...
    def call_model(self, model: torch.nn.Module, images: torch.Tensor, text: torch.Tensor, image_features: torch.Tensor = None, text_features: torch.Tensor = None) -> torch.Tensor:
        ...
        if text_features is None:
            ...
            # input()
        if self.args.TD:
            num_text = text.shape[0]
            if "shade" in self.box_representation_method:
                redundant_features = model.encode_text(self.preprocess_text([" is in red color."]*num_text).to(self.device))
            elif self.args.text_prompt == 'a photo of <caption>':
                redundant_features = model.encode_text(self.preprocess_text(["a photo of "]*num_text).to(self.device))
            elif self.args.text_prompt == 'This is <caption>':
                redundant_features = model.encode_text(self.preprocess_text(["This is "]*num_text).to(self.device))
            elif self.args.text_prompt == 'None':
                redundant_features = model.encode_text(self.preprocess_text([""]*num_text).to(self.device))
            elif self.args.text_prompt == 'a photo of a <caption>':
                redundant_features = model.encode_text(self.preprocess_text(["a photo of a"]*num_text).to(self.device))
        ...
        per_txt_sm = clip.clip_feature_surgery(image_features, text_features, redundant_feats=None)
        # print(per_txt_sm.shape)
        # print(logits_per_image.shape)
        # logits_per_text = logits_per_image.t()
        logits_per_text = logits_per_image.permute(2, 1, 0)
        # print(logits_per_text.shape)

        return logits_per_image, logits_per_text, image_features, text_features, per_txt_sm.permute(2, 1, 0)
